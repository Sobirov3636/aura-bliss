...
import { ExtendedRequest, Member, MemberInput, MemberUpdateInput } from "../libs/types/member";
...
memberController.signup = async (req: Request, res: Response) => {
  try {
    ...
    res.status(HttpCode.CRATED).json({ member: result, accessToken: token });
  } catch (err) {
...
memberController.verifyAuth = async (req: ExtendedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!req.member) throw new Errors(HttpCode.UNAUTHORIZED, Message.NOT_AUTHENCTICATED);
